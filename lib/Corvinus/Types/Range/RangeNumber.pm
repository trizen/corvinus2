package Corvinus::Types::Range::RangeNumber {
    ...
    *includes = \&contains;
        ...
        if ($step == 1 and $limit < (-1 >> 1) and $from < (-1 >> 1)) {
            ...
            if (ref($from) || ref($limit) || ref($step)) {
                ...
                for (my $i = $from->copy ; $i->bcmp($limit) <= 0 ; $i->badd($step)) {
                    if (defined(my $res = $code->_run_code(Corvinus::Types::Number::Number->new($i->copy)))) {
                        return $res;
                    }
                }
            }
    ...
    sub map {
        ...
        if ($step == 1 and $limit < (-1 >> 1) and $from < (-1 >> 1)) {
            ...
            foreach my $i ($from .. $limit) {
                push @values, $code->run(Corvinus::Types::Number::Number->new($i));
            }
        }
    ...
    sub grep {
        ...
        if ($step == 1 and $limit < (-1 >> 1) and $from < (-1 >> 1)) {
            ...
            foreach my $i ($from .. $limit) {
                my $num = Corvinus::Types::Number::Number->new($i);
